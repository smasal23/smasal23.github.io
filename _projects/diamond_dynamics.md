...
description: End-to-end ML pipeline for predicting diamond prices and identifying segmented market tiers.
img: assets/img/projects/diamond/streamlit_home.png
importance: 1
category: Machine Learning & AI
---

**[View Source Code on GitHub](https://github.com/smasal23/projects/tree/547e3a5f08a3cd15e9b222b894328834ce784be2/Diamond%20Dynamics%20Price%20Prediction%20and%20Market%20Segmentation)** | **[Live Streamlit Dashboard](YOUR_STREAMLIT_APP_LINK)**
...
* **Machine Learning:** `Scikit-Learn`, `XGBoost`, `LightGBM`, `PyTorch` (ANN branch)
* **Ops & Deployment:** `Streamlit`, `Joblib`, `PyYAML`, `Pytest`
* **System Architecture:** Config-driven parameters (`configs/`), reusable modules (`src/data/`, `src/features/`, `src/modeling/`), and an isolated inference layer (`src/inference/`).
...
* **Derived Metrics:** Engineered domain-specific features including `volume_proxy` (x * y * z), `length_width_ratio`, and `table_depth_interaction`.
...
| **RMSE** | **$542.38** | Average prediction error margin is highly competitive for retail. |
| **MAE** | **$274.76** | Median absolute error showcases strong baseline reliability. |
...
The app validates user input, dynamically computes the engineered features (like `table_to_depth_ratio`), and feeds the data through the saved artifact pipelines (`.pkl` models and scalers) to instantly output:
